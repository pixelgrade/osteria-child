<?php
/**
 * Osteria Child functions and definitions
 *
...
/**
 * Setup Osteria Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function osteria_child_theme_setup() {
	load_child_theme_textdomain( 'osteria-child-theme', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'osteria_child_theme_setup' );
...
function osteria_child_enqueue_styles() {
	$theme = wp_get_theme();
	$parent = $theme->parent();

	if ( is_rtl() ) {
		wp_enqueue_style( 'osteria-rtl-style', get_template_directory_uri() . '/rtl.css', array(), $parent->get( 'Version' ) );
	} else {
		wp_enqueue_style( 'osteria-main-style', get_template_directory_uri() . '/style.css', array(), $parent->get( 'Version' ) );
	}

	// Here we are adding the child style.css while still retaining
	// all of the parents assets (style.css, JS files, etc)
	wp_enqueue_style( 'osteria-child-style',
		get_stylesheet_directory_uri() . '/style.css',
		array('osteria-main-style'), //make sure the the child's style.css comes after the parents so you can overwrite rules
		$parent->get( 'Version' )
	);
}
add_action( 'wp_enqueue_scripts', 'osteria_child_enqueue_styles' );
...
function osteria_child_overwrite_files() {
	...
	// See the parent's function.php -> the osteria_scripts_styles() function
	// for details like resources names

		wp_dequeue_script( 'osteria-main-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'osteria-child-scripts',
	...
	// see the parent's function.php -> the hive_scripts_styles() function for details like resources names
		...
		wp_dequeue_style( 'osteria-main-style' );
...
// Load the files from the function mentioned above:
	...
	add_action( 'wp_enqueue_scripts', 'osteria_child_overwrite_files', 11 );
